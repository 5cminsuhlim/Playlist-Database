...
def user_podcast_subscriptions(username):
    ...
    try:
        ...
        sql = """SELECT podcast_id, podcast_title, podcast_uri, podcast_last_updated, COUNT(podcast_id)
...
def user_in_progress_items(username):
    ...
    try:
        ...
        sql = """SELECT media_id, play_count as playcount, progress, lastviewed, storage_location
...
def get_alltvshows():
    ...
    try:
        ...
        #############################################################################
        sql = """SELECT tvshow_id, tvshow_title, COUNT(episode)
...
def get_song(song_id):
    ...
    try:
        ...
        FROM mediaserver.Song INNER JOIN mediaserver.Song_Artists USING (song_id)
...
def get_song_metadata(song_id):
    ...
    try:
        ...
        sql = """
        SELECT md_type_name, md_value AS md_value ,md_id
        FROM mediaserver.MediaItemMetaData FULL OUTER JOIN mediaserver.MetaData USING (md_id) 
        INNER JOIN mediaserver.MetaDataType USING (md_type_id)
        WHERE media_id = %s
        UNION 
        SELECT md_type_name, md_value, md_id
        FROM mediaserver.Song INNER JOIN mediaserver.Album_Songs USING (song_id)
        ...
        UNION 
        SELECT md_type_name, md_value, md_id
        FROM mediaserver.Song_Artists INNER JOIN mediaserver.ArtistMetaData ON (performing_artist_id = artist_id)
        INNER JOIN mediaserver.MetaData USING (md_id)
        INNER JOIN mediaserver.MetaDataType USING (md_type_id)
        WHERE song_id = %s
        """
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT podcast_title, podcast_id, podcast_uri, podcast_last_updated, md_type_name, md_value, md_id
...
def get_all_podcasteps_for_podcast(podcast_id):
    ...
    try:
        ...
        sql = """
        SELECT media_id, podcast_episode_title, podcast_episode_uri, podcast_episode_published_date, podcast_episode_length,md_id
        ...
        WHERE podcast_id = %s
        GROUP BY media_id, podcast_episode_title, podcast_episode_uri, podcast_episode_published_date, podcast_episode_length,md_id
...
def get_podcastep(podcastep_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT podcast_episode_title, media_id AS podcast_episode_id, podcast_episode_uri, podcast_episode_published_date, podcast_episode_length,
        md_type_name, md_value
        FROM mediaserver.Podcast INNER JOIN mediaserver.PodcastEpisode USING (podcast_id)
        INNER JOIN mediaserver.MediaItemMetaData USING (media_id)
        INNER JOIN mediaserver.MetaData USING (md_id)
        INNER JOIN mediaserver.MetaDatatype USING (md_type_id)
...
def get_album(album_id):
    ...
    try:
        ...
        FROM mediaserver.MetaData INNER JOIN mediaserver.MetaDataType USING (md_type_id)
        INNER JOIN mediaserver.AlbumMetaData USING (md_id)
        INNER JOIN mediaserver.Album USING (album_id)
        WHERE album_id = %s
        """
...
def get_album_songs(album_id):
    ...
    try:
        ...
        INNER JOIN mediaserver.Song ON (media_id = song_id)
...
def get_genre_songs(genre_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT DISTINCT a.song_id ,a.song_title, a.length,e.md_type_name, d.md_value, d.md_id
                        FROM mediaserver.song a INNER JOIN mediaserver.album_songs b USING (song_id)
                		INNER JOIN mediaserver.audiomedia f ON (a.song_id= f.media_id)
                		JOIN mediaserver.mediaitemmetadata g ON (f.media_id = g.media_id)
                        	 JOIN mediaserver.metadata d ON (g.md_id = d.md_id)
                			 JOIN mediaserver.metadatatype e ON (d.md_type_id = e.md_type_id)
        WHERE d.md_id = %s
        """
...
def get_genre_podcasts(genre_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT DISTINCT a.*, c.md_id, c.md_value
        FROM mediaserver.podcast a INNER JOIN mediaserver.podcastmetadata g USING (podcast_id)
                			INNER JOIN mediaserver.podcastepisode b USING (podcast_id)
                			JOIN mediaserver.mediaitemmetadata f ON (b.media_id = f.media_id)
                        	JOIN mediaserver.metadata c ON (c.md_id = f.md_id)
                			JOIN mediaserver.metadatatype d ON (c.md_type_id = d.md_type_id)
        WHERE c.md_id = %s
        """
...
def get_genre_movies_and_shows(genre_id):
    ...
    try:
        ...
        SELECT DISTINCT a.movie_id, a.movie_title, a.release_year,g.tvshow_id, g.tvshow_title, e.md_value, f.md_id
                FROM mediaserver.movie a JOIN mediaserver.videomedia b ON (a.movie_id = b.media_id)
                	INNER JOIN mediaserver.mediaitem c USING (media_id)
                	INNER JOIN mediaserver.mediaitemmetadata d USING (media_id)
                	INNER JOIN mediaserver.metadata e USING (md_id)
                	LEFT OUTER JOIN mediaserver.tvshowmetadata f ON (e.md_id = f.md_id)
                	JOIN mediaserver.tvshow g ON (f.tvshow_id = g.tvshow_id)
        WHERE e.md_id = %s
           OR f.md_id = %s
...
def get_tvshow(tvshow_id):
    ...
    try:
        ...
        # including all relevant metadata       #
        #############################################################################
        sql = """SELECT tvshow_title, md_value, md_type_name, md_id
        FROM mediaserver.MetaData INNER JOIN mediaserver.MetaDataType USING (md_type_id)
...
def find_matchingmovies(searchterm):
    ...
    try:
        ...
        #FROM mediaserver.Movie INNER JOIN mediaserver.MediaItem ON (movie_id = media_id)
	    #INNER JOIN mediaserver.MediaItemMetaData USING (media_id)
	    #INNER JOIN mediaserver.MetaData USING (md_id)
	    #INNER JOIN mediaserver.MetaDataType USING (md_type_id)
	    #WHERE movie_title = %s
...
#   Add a new Song
#####################################################
def add_song_to_db(title, storage_location, description, length, genre, artist_id):
    """
    Get all the matching Movies in your media server
    """
    #########
