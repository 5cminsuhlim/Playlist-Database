...
def single_genre(genre_id):
    ...
    page['title'] = 'List single genre' # Add the title
    ...
    songs = None # will add all songs that match
    songs = database.get_genre_songs(genre_id)
    
    films = None # will add all films that match
    films = database.get_genre_movies_and_shows(genre_id)

    podcasts = None # will add all podcast that match 
...
def add_song():
    ...
    if request.method == 'POST':
        ...
        if 'genre' not in request.form:
            newdict['genre'] = 'Empty song genre'
        else:
        ...
        songs = database.add_song_to_db(newdict['song_title'],newdict['storage_location'],newdict['description'],newdict['length'],newdict['genre'],newdict['artist_id'])
